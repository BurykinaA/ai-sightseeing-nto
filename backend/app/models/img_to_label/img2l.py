...
class Model:
    def __init__(self, city) -> None:
        ...
        pretrained_model_ekb = torch.hub.load(
            "pytorch/vision:v0.10.0", "mobilenet_v2", pretrained=False
        )
        pretrained_model_ekb.classifier = nn.Sequential(
            nn.Dropout(p=0.2, inplace=False),
            nn.Linear(
                in_features=pretrained_model_ekb.last_channel,
                out_features=num_classes,
                bias=True,
            ),
        )
        self.model = pretrained_model_ekb.load_state_dict(
            torch.load(models[self.city])
        ).to(device)
