...
class AutoCompleter:
    ...
    def search(self, query: str, size=5, max_cost=2, translated=False) -> List[str]:
        outputs = self.autoComplete.search(
            word=self.processString(query, do_print=True), size=size, max_cost=max_cost
        )
        output = [item[0] for item in outputs]
        if not output and not translated:
            if self.corrector.isEnglish(query):
                output = self.search(self.corrector.en2ru(query), size, max_cost, True)
            else:
                output = self.search(self.corrector.ru2en(query), size, max_cost, True)
        if not output:
